...
class Dom
{
    public function __construct() 
    {
        ...
        $this->installPath              = '/root/gitprojects/sslinstall';
        ...
        $this->testing->path            = $this->installPath . '/test';
        ...
        if ($this->testing->testMode == true){
            ...
            $this->csrInputData     =   json_decode(
                file_get_contents(
                    $this->testing->path . "/testCsrInputData.json"));

            $this->domainData       = json_decode(
                file_get_contents(
                    $this->testing->path . "/testDomainData.json"));

            $this->csrData          = json_decode(
                file_get_contents(
                    $this->testing->path . "/testCsrData.json"));

            $this->csrHashes        = json_decode(
                file_get_contents(
                    $this->testing->path . "/testCsrHashes.json"));

        } else {

            $this->domainData       = $this->whm1->getDomainData(
                                        $this->csrInputData->domainName);

            $this->csrData          = $this->whm1->getCsrData(
                                        $this->csrInputData);

            $this->csrHashes        = $this->com->getCsrHashes(
                                        $this->csrData->data->csr);
        }

        $this->dcv             = new \stdClass();
        $this->dcv->subdir     = "/.well-known/pki-validation";
        $this->docRoot         =$this->domainData->data->userdata->documentroot;
        $this->dcv->dir        = $this->docRoot . $this->dcv->subdir;
        $this->dcv->fileName   = "/" . $this->csrHashes->md5 . ".txt";
        $this->dcv->filePath   = $this->dcv->dir . $this->dcv->fileName;
        $this->dcv->url        = $this->csrInputData->domainName 
                                    . $this->dcv->subdir . $this->dcv->fileName;

        $this->dcv->httpUrl    = "http://" . $this->dcv->url;
        $this->dcv->httpsUrl   = "https://" . $this->dcv->url;
        $this->dcv->dcvContent = $this->csrHashes->sha256 
                                    . " comodoca.com\n" 
        ...
        if ($this->testing->testMode == true){
            ...
            $this->sslOrder     = json_decode(
                                    file_get_contents(
                                        $this->testing->path 
                                        . "/testSslOrder.json")); 
            $this->certificate   = json_decode(
                                        file_get_contents(
                                            $this->testing->path 
                                            . "/testCertificate.json"));
        } else {

            $this->sslOrder      = $this->com->orderSsl(
                                        $this->csrData->data->csr);

            $this->certificate   = $this->com->collectSsl(
                                        $this->sslOrder);
        }
    ...
    public function getFingerPrint() 
    {
        ...
        $shellExecStr   = "openssl x509 -noout " 
